...
- **`known_keys` stores each entry's full fields, not just a seen-flag, and extraction can return either "new since last time" or "everything known" (`want_all`).** The real push to AnnotationSync should only send what's new; a debug/inspection tool should show the complete current picture regardless of what happened to already be recorded from an earlier run. Conflating the two the first time this was built meant running the debug "extract" action before "dump to file" made the dump come back empty — the extract call had already marked everything as seen.
